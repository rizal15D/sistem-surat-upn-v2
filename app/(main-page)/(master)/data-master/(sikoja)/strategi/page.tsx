...
import { Strategi, columns } from "./columns";
...
async function getData(): Promise<Strategi[]> {
...
export default function DataMasterRolePage() {
  ...
  const { mutate } = useMutation({
    mutationFn: async (input: { name: string }) => {
      setIsLoading(true);
      const response = await axios.post(`/api/sikoja/strategi/`, { input });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["strategi"] });
    ...
    <>
      ...
      {modalCreateOpen && (
        <Modal setModalOpen={setModalCreateOpen}>
          <StrategiForm onSubmit={handleCreate} isLoading={isLoading} />
        </Modal>
      )}
    </>
  );
